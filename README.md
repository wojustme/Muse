...
项目采用 `pnpm` monorepo，包含 Fastify Server、Tauri Desktop、Tauri Mobile 和多个共享 TypeScript 包。本文只描述当前代码中已经存在的能力；仅有设计文档或类型预留、尚未接入产品主流程的能力会明确标注。
...
| 领域          | 已实现能力                                                                   |
| ------------- | ---------------------------------------------------------------------------- |
| 客户端        | macOS Desktop、Tauri Mobile（iOS 已配置，Android 保留初始化与构建入口）      |
...
1. 客户端层
   ...
   `apps/desktop` 和 `apps/mobile` 负责登录态恢复、模型选择、会话列表、消息历史、Markdown 渲染和 Chat SSE 流消费。Desktop 额外初始化 `LocalToolBridge`；Mobile 额外查询在线 Desktop 并选择远程工作区。
...
```mermaid
flowchart LR
  Desktop[Desktop React] -->|HTTP + SSE| Server[Fastify Server]
...
pnpm --filter @muse/mobile run build
```
...
- Desktop、Mobile 仍分别维护大体量 `App.tsx`，部分功能可能发生漂移。
